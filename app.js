  ...
  if (isPageLoaded === false) {
    ...
    function moveCarouselLeft() {
      const currentWidth = Number(
        projectsDiv.style.transform.replace("vw", "")
      );
      projectsDiv.style.transform = `translateX(${currentWidth - stepWidth}vw)`;
    }

    // Add an event listener to the left arrow (or any other trigger)
    leftArrow.addEventListener("click", (event) => {
      moveCarouselLeft();
    });
    rightArrow.addEventListener("click", (event) => {
      projectsDiv.style.transform = "translateX(-5vw)";
    });
  }
  isPageLoaded = true;
};
